...
###  Task 3.1 Please instead of using Hardcoded Columns use Pandas Dataframe to get  ordered_numbers_df this data from there
    ...
    columns = ordered_numbers_df.columns.str.lower()
    
    dictavro["type"] = "record"
    dictavro["namespace"] = table_name + ".avro"
    dictavro["name"] = table_name.lower()
    dictavro["fields"] = [{"name": columns[0], "type": "int"},
                          {"name": columns[1], "type": "int"}, 
                          {"name": columns[2], "type": "int"},
                          {"name": columns[3], "type": "int"},
                          {"name": columns[4], "type": "int"},
                          {"name": columns[5], "type": "int"}, 
                          {"name": columns[6], "type": "int"},
                          {"name": columns[7], "type": "int"},
                          {"name": columns[8], "type": "int"},
                          {"name": columns[9], "type": "int"},
                          {"name": columns[10], "type": "int"},
                          {"name": columns[11], "type": "string"}, 
                          {"name": columns[12], "type": "string"}]
...
def write_dataframe_to_json(ordered_numbers_df, json_file_name):
    ...
    ordered_numbers_df.to_json('myjson.json', orient='records', lines=True)
    json.dumps({})
...
def slice_data_frame_by_condition(in_df):
    df = in_df.drop(in_df[in_df.COLUMN1_INT > 20].index)
    df = df.drop(df[df.COLUMN2_INT > 20].index)
    df = df.drop(df[df.COLUMN3_INT > 20].index)
    df = df.drop(df[df.COLUMN4_INT > 20].index)
    df = df.drop(df[df.COLUMN5_INT > 20].index)
    df = df.drop(df[df.COLUMN6_INT > 20].index)
    df = df.drop(df[df.COLUMN7_INT > 20].index)
    df = df.drop(df[df.COLUMN8_INT > 20].index)
    df = df.drop(df[df.COLUMN9_INT > 20].index)
    out_df = df.drop(df[df.COLUMN10_INT > 20].index)    
    return out_df
...
if __name__ == "__main__":
    ...
    input_dataframe = slice_data_frame_by_condition(input_dataframe)
